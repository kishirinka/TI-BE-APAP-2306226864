...
class ClaimServiceTest {
    ...
    void testCreateClaim_Success() {
        ...
        // Act
        Claim result = claimService.createClaim(newClaim, "OP1");

        // Assert
        assertNotNull(result);
        assertEquals("CLM11", result.getId()); // count + 1 = 11
        assertEquals(ClaimStatusEnum.WAITING_FOR_REVIEW, result.getStatus());
        assertEquals("medical-report.pdf", result.getProof());
        assertEquals(testOrderedPlan, result.getOrderedPlan());
        assertNotNull(result.getCreatedAt());
        assertNotNull(result.getUpdatedAt());

        // Verify ordered plan status updated
        assertEquals(OrderedPlanStatusEnum.WAITING_FOR_REVIEW, testOrderedPlan.getStatus());

        verify(orderedPlanService, times(1)).getOrderedPlanById("OP1");
        verify(claimRepository, times(1)).count();
        verify(claimRepository, times(1)).save(any(Claim.class));
        verify(orderedPlanRepository, times(1)).save(testOrderedPlan);
    }

    // ========================== TEST CREATE CLAIM - FAIL (ORDERED PLAN NOT PAID) ==========================
    @Test
    void testCreateClaim_Fail_OrderedPlanNotPaid() {
        // Arrange
        testOrderedPlan.setStatus(OrderedPlanStatusEnum.ORDERED); // Not PAID yet
        ...
        RuntimeException exception = assertThrows(RuntimeException.class, () -> {
            claimService.createClaim(newClaim, "OP1");
        });

        assertTrue(exception.getMessage().contains("OrderedPlan must be PAID"));
        verify(orderedPlanService, times(1)).getOrderedPlanById("OP1");
        verify(claimRepository, never()).save(any(Claim.class));
        verify(orderedPlanRepository, never()).save(any(OrderedPlan.class));
    }

    // ========================== TEST CREATE CLAIM - FAIL (ALREADY EXPIRED) ==========================
    @Test
    void testCreateClaim_Fail_AlreadyExpired() {
        // Arrange
        testOrderedPlan.setExpiredDate(LocalDate.now().minusDays(10)); // Expired 10 days ago
        ...
        assertTrue(exception.getMessage().contains("OrderedPlan has expired"));
        verify(orderedPlanService, times(1)).getOrderedPlanById("OP1");
        verify(claimRepository, never()).save(any(Claim.class));
        verify(orderedPlanRepository, never()).save(any(OrderedPlan.class));
    }
